...
function CardArtist({ id, imageUrl, artistName, artistType }) {
  ...
  return (
    <div onClick={() => switchDetailArtist()} className="bg-[#131212] rounded-lg w-[15%] mr-5 p-2 relative group">
      <img src={imageUrl} alt={artistName} className="rounded-full w-40 h-36 mx-auto mb-4" />
      <div className="absolute inset-0 flex items-center justify-center bg-[#323232] bg-opacity-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
