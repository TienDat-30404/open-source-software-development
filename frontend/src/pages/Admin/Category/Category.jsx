...
function Category() {
    ...
    const { data: categories, isLoading, isError, error } = useGetAllCatgory(query)
    ...
    return (
        <div className="bg-gray-100 min-h-screen p-6">
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {categories && categories?.results?.length > 0 && categories?.results?.map((category, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                                        <button className="text-indigo-600 hover:text-indigo-900 focus:outline-none"
                                            onClick={() => handleSelectedArtist(category?.id)}
                                        >
                                            <MoreHorizontal />
                                        </button>
                                        {selectedArtist === category?.id && (
                                            <div className="absolute right-6 top-9 mt-2 p-3 rounded bg-gray-500 shadow text-white z-10">
                                                <h2
                                                    className="cursor-pointer hover:underline"
                                                    onClick={() => handleSelectedArtistEdit(category)}
                                                >
                                                    Update
                                                </h2>
            ...
            {categories && categories?.total_pages > 1 && (
                <Pagination
                    totalPage={categories?.total_pages}
            ...
            <AddCategoryModal show={showModalAddCategory} onClose={() => setShowModalAddCategory(false)} />
            <EditCategoryModal show={showModalEditCategory} onClose={() => setShowModalEditCategory(false)} data={dataCategory} />
