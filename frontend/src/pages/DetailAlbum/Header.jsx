        ...
        <div className=" w-full h-64 rounded-lg overflow-hidden bg-gradient-to-l  from-[#378438e1] to-[#9517174a]">
            <div className=" w-full h-full flex">
                ...
                <div className="flex flex-col justify-center " >
                    <p className="text-xs uppercase text-white font-medium">Album</p>
                    <h1 className="text-6xl font-bold text-white pt-3">{details?.title}</h1>
                    <div className='flex items-center pt-3 space-x-2'>
                        <img
                            src="https://tse4.mm.bing.net/th?id=OIP.T3MGuXvCM6uZa2xpEI-nOAHaEf&pid=Api&P=0&h=180"
                            className='w-5 h-5 rounded-full'
                        />
                        <p className="text-sm text-gray-300">buitruonglinh</p>
                        <div className='w-1 h-1 bg-yellow-100 rounded-full'></div>
                        <p className="text-sm text-gray-300">{new Date(details?.release_date).getFullYear()}</p>
                        <div className='w-1 h-1 bg-yellow-100 rounded-full'></div>
                        <p className="text-sm text-gray-300">{details?.songs?.length} songs</p>
                        <div className='w-1 h-1 bg-yellow-100 rounded-full'></div>
                        <p className="text-sm text-gray-300">
                            {switchDurationVideo(details?.songs?.reduce((acc, cur) => acc + cur.duration, 0))}
                        </p>
